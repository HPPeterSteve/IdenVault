 ...
 * Sections 8, 9, 10, 11 from diamondVaults.c
 *
 * Contains:
 ...
 *
 *  Files filtered:
 *    .diamond_jail_ready  — jail marker written by vault_prepare_jail()
...
static inline bool is_sandbox_internal(const char *name) {
    ...
    if (strcmp(name, SANDBOX_JAIL_MARKER) == 0)   return true;  /* .diamond_jail_ready */
