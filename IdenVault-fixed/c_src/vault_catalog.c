...
void resolve_paths(void) {
    ...
    snprintf(g_catalog_path, sizeof(g_catalog_path), "%s/.local/share/idenvault", home);
    snprintf(g_catalog_file, sizeof(g_catalog_file), "%s/.local/share/idenvault/catalog.dat", home);
    snprintf(g_log_file, sizeof(g_log_file), "%s/.local/share/idenvault/vault_security.log", home);
    snprintf(g_lock_file, sizeof(g_lock_file), "%s/.local/share/idenvault/vault_security.pid", home);
    ...
    if (stat(g_catalog_file, &st) != 0 && stat(legacy_catalog, &st) == 0) {
        ...
        if (src) {
            ...
            if (dst) {
                ...
                vault_log(LOG_INFO, "Migracao concluida com success.");
            } else {
                vault_log(LOG_ERROR, "Failure ao criar o novo catalog.dat em %s", g_catalog_file);
            }
            fclose(src);
        } else {
            vault_log(LOG_WARN, "Nao foi possivel ler o catalog.dat legado (falta de permissao?)");
        }
    }
}
                        ...
                        const char *path_arg, const char *password)
{
    ...
    if (chmod(v->cipher_path, 0000) != 0) {
        ...
        vault_log(LOG_AUDIT,
                  ...
                  "State: FULLY ISOLATED. Only IdenVault daemon can unlock via FUSE or export.",
