...
int vault_encrypt_ffi(uint32_t id, const char *password) {
    ...
    uint8_t key[KEY_LEN];
    VaultErrorr key_err = derive_key(password, v->salt, key);
    if (key_err != ERR_OK) {
#ifdef __linux__
        pthread_mutex_unlock(&g_monitor.lock);
#endif
 ...
 * ═══════════════════════════════════════════════════════════════════════════
 *
 *  vault_worm_set_flags_ffi(id, flags)   — OR the given WORM_PROTECT_* bits in
 *  vault_worm_clear_flags_ffi(id, flags) — AND-NOT the given bits out
 *  vault_worm_set_scan_ffi(id)           — engage full PROTECTED-SCAN (irreversible)
 *  vault_worm_get_flags_ffi(id)          — read current bitmask (0 on error)
 *
 *  All functions return 0 on success or a negative VaultErrorr code.
 * ─────────────────────────────────────────────────────────────────────────── */
...
/* ═══════════════════════════════════════════════════════════════════════════
 *  mount-export — the ONLY sanctioned egress path for PROTECTED-SCAN vaults
 * ═══════════════════════════════════════════════════════════════════════════
 *
 *  vault_mount_export_ffi(id, password, dst_dir, filename)
 *
 *    id        — vault ID
 *    password  — vault password (required for VAULT_TYPE_PROTECTED; may be
 *                NULL/"" for VAULT_TYPE_NORMAL vaults)
 *    dst_dir   — destination directory (must already exist)
 *    filename  — file to export, or NULL / "" meaning "export all files"
 *
 *  For PROTECTED-SCAN vaults the operation is audited and does NOT require
 *  unmounting first — it reads directly from cipher_path bypassing FUSE so
 *  that the WORM layer cannot interfere.  For normal vaults it is equivalent
 *  to vault_export_file_ffi but with richer logging.
 *
 *  Returns 0 on success or a negative VaultErrorr code.
 * ─────────────────────────────────────────────────────────────────────────── */
                           ...
                           const char *dst_dir, const char *filename)
{
    ...
    while ((de = readdir(dir)) != NULL) {
        ...
        if (v->type == VAULT_TYPE_PROTECTED &&
            ...
            uint8_t key[KEY_LEN];
            VaultErrorr kerr = derive_key(password, v->salt, key);
            if (kerr != ERR_OK) {
                vault_log(LOG_ERROR,
 ...
 *  Simple linked-list protected by the monitor mutex.
 * ═══════════════════════════════════════════════════════════════════════════ */
...
int vault_auth_pid_is_authorized_ffi(pid_t pid) {
    /* IdenVault itself is always authorized */
